...
* Output to Word via putdocx (Lags chosen via Sequential t-test at 10%)
* ==============================================================================
clear all
set more off
...
putdocx text ("Zivot-Andrews Unit Root Analysis (Sequential t-test)")

putdocx paragraph
putdocx text ("This document presents the results of the Zivot-Andrews structural break unit root tests, restricting the sample to 1909 and later, with lag length chosen via the sequential t-test (10% level).")
...
putdocx text ("Table 1: Zivot-Andrews Endogenous Break Results (t-test)")
...
putdocx table za_results(1,4) = ("Lags (t-test)")
...
foreach v of local vars_both {
    * lagmethod(t) automatically applies the 10% sequential t-test
    quietly zandrews `v', break(both) lagmethod(TTest)
...
foreach v of local vars_int {
    quietly zandrews `v', break(intercept) lagmethod(TTest)
...
putdocx paragraph
putdocx text ("Yes, there are minor differences because of the lag selection method. In the previous exercise, lag lengths were chosen to minimize an information criterion (like AIC or BIC). Here, lag lengths are chosen using a sequential t-test (dropping lags until the last one is significant at the 10% level). Because the chosen lag length alters the underlying regression, it can shift the algorithm's optimal 'worst-case' break year and change the resulting minimum t-statistic for several of the variables.")

putdocx paragraph
putdocx text ("Are your results different from those in Zivot and Andrews' paper?"), bold
putdocx paragraph
putdocx text ("Yes. Zivot and Andrews (1992) utilized the full historical Nelson-Plosser dataset (dating back to 1860 for some variables). By dropping all observations prior to 1909, we have severely restricted the search window. Consequently, the algorithm is forced to identify different, sub-optimal break dates (and therefore different test statistics) for variables that originally had structural breaks in the late 19th century.")

* 6. Save and Close
putdocx save "NelsonPlosser_ZA_ttest.docx", replace
disp "Document successfully created: NelsonPlosser_ZA_ttest.docx"
