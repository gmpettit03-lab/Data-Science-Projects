...
* Output to Word via putdocx (Lags chosen via BIC)
* ==============================================================================
clear all
set more off
...
putdocx paragraph
putdocx text ("This document presents the results of the Zivot-Andrews structural break unit root tests, restricting the sample to 1909 and later, with lag length chosen via BIC.")
...
putdocx table za_results(1,4) = ("Lags (BIC)")
...
foreach v of local vars_both {
    quietly zandrews `v', break(both) lagmethod(BIC) maxlags(8)
...
foreach v of local vars_int {
    quietly zandrews `v', break(intercept) lagmethod(BIC) maxlags(8)
